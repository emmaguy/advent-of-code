...
class IntcodeComputerTest {
    ...
    @Test fun `can add in memory`() {
        val memory = arrayOf(1, 0, 0, 0, 99)
        IntcodeComputer(memory = memory, inputList = mutableListOf()).apply { compute() }

        assertArrayEquals(arrayOf(2, 0, 0, 0, 99), memory)
    }

    @Test fun `can multiply in memory`() {
        val memory = arrayOf(2, 3, 0, 3, 99)
        IntcodeComputer(memory = memory, inputList = mutableListOf()).apply { compute() }

        assertArrayEquals(arrayOf(2, 3, 0, 6, 99), memory)
    }

    @Test fun `can do basic operations in memory`() {
        val memory = arrayOf(1, 1, 1, 4, 99, 5, 6, 0, 99)
        IntcodeComputer(memory = memory, inputList = mutableListOf()).apply { compute() }

        assertArrayEquals(arrayOf(30, 1, 1, 4, 2, 5, 6, 0, 99), memory)
    }
    ...
    @Test fun `can handle position mode`() {
        ...
        assertArrayEquals(arrayOf(1002, 4, 3, 4, 99), memory)
    }
    ...
    @Test fun `position mode input equal to 8`() {
        ...
        assertEquals(computer.output(), listOf(1))
    }
    ...
    @Test fun `position mode input not equal to 8`() {
        ...
        assertEquals(computer.output(), listOf(0))
    }
